...
const updatePosition = async (diceRoll, regNo, from) => {
  try {
    ...
    if (diceRoll !== 0) {
      const query =
        from === "dice-roll"
          ? { $inc: { currPosition: diceRoll, totalRolls: 1 } }
          : { $set: { currPosition: diceRoll }, $inc: { totalRolls: 1 } };
      const user = await userModel.findOneAndUpdate({ regNo: regNo }, query, {
        new: true,
      });
...
const updateUserRollValue = async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Error while updating dice roll value",
      status: false,
    });
  }
};
...
const updateContestTimer = async (req, res) => {
  ...
  if (userMatch.isAdmin) {
    ...
    try {
      ...
      if (reqDoc) {
        reqDoc = await reqModel.findOneAndUpdate(
          { id: "requirements" },
...
const setScoreZero = async (regNo) => {
  try {
    await userModel.findOneAndUpdate(
      { regNo: regNo },
      { $set: { score: 0, currPosition: 1, totalRolls: 0, rollValues: [] } }
    );
    return true;
  } catch (error) {
    console.log(error.message);
